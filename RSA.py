...
class RSA:
    ...
    def BLOCK_SIZE(self): # floor(n/8)
        return math.ceil(self.__bits / 8) - 1
    ...
    def encrypt(self, message): # TODO: Brahim
        ciphertext = []
        message = self.__pad(message, self.BLOCK_SIZE)
        blocks = self.__split_into_blocs(message, self.BLOCK_SIZE)
        for block in blocks:
            block_int = self.__encode(block)
            c = PrimeUtils.square_n_multiply(block_int, self.e, self.n)
            ciphertext.append(self.__decode(c, self.BLOCK_SIZE+1))
    ...
    def decrypt(self, ciphertext): # TODO: Brahim
        message = []
        blocks = self.__split_into_blocs(ciphertext, self.BLOCK_SIZE+1)
        for block in blocks:
            ...
            message.append(self.__decode(m, self.BLOCK_SIZE))
    ...
    def sign(self, certificate_tbs_DER):
        ...
        return self.__decode(c, self.BLOCK_SIZE+1)
    ...
    def verifySignature(self, certificate_tbs, signature):
        ...
        c = PrimeUtils.square_n_multiply(block_int, self.e, self.n)
        decrypted_signature = self.__unpad(self.__decode(c, self.BLOCK_SIZE))
